...
path.res      <- "/glade/campaign/mmm/c3we/ESTCP/CMIP6_downscaled_statistical/AK_10km/"

path.out      <- "/glade/work/taereemk/AK_10km_Timeseries/"
...
for(im in 1:length(model.list)){
  ...
  for (iyr in 1:length(yr)){
    ...
    file_path <- paste0(path.res, model.list[im],"/pr_new/bc_pr_day_",model.list[im],"_historical_",yr[iyr],"0101-",yr[iyr],"1231_",res.list.na,".nc")
