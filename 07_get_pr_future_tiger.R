...
# Bias correction methods list for temperature and precipitation 
bc.list <- c("obs.model","dqm","eqm","gpqm","qdm","scaling","pqm","mva","loci", "ptr")

path.res      <- "/scratch/gpfs/GVILLARI/tk6869/AK_10km/"
path.out      <- "/scratch/gpfs/GVILLARI/tk6869/AK_10km_Timeseries/"
dir.create(path.out, showWarnings = F)
path.out.1 <- paste0(path.out, cvar, "/")
dir.create(path.out.1, showWarnings = F)

# Initialize parallel backend (only once)
...
for(im in 1:10){
  ...
  for(is in 1:length(scenario)){
    ...
    # Loop through each bias correction method and scenario
    for(ires in 1:length(bc.list)){
      ...
      regional.avg.future_list <- foreach(iyr = 1:length(yr.f), .packages = c("raster", "dplyr", "lubridate")) %dopar% { 
        ...
        file_path <- paste0(path.res, model.list[im], "/",cvar,"_new/bc_",cvar,"_day_", model.list[im], "_", scenario[is], "_", yr.f[iyr], "0101-", yr.f[iyr], "1231_", bc.list[ires], ".nc")
